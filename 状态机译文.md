...
一个轻量的、面对对象的、有许多扩展的 python 状态机实现。 可在 python2.7 和 3.0+下使用。
...
有人说[一个好的例子比得上](https://www.google.com/webhp?ie=UTF-8#q=%22a+good+example+is+worth%22&start=20)100 页的 api 手册。一百万条指导，或者一千句话。

其实说的不全对...但这里还是提供了一个例子：
...
class NarcolepticSuperhero(object):
    ...
    # 定义一些状态。大部分时候，嗜睡的超级英雄和其他人差不多。除了...
    states = ['asleep', 'hanging out', 'hungry', 'sweaty', 'saving the world']

    def __init__(self, name):

        # 没有匿名的超级英雄！所有的超级英雄都有一个名字。任何名字都可以。
        # SleepyMan. SlumberGirl. 你懂的。
        self.name = name
        ...
        # 添加一些转换。我们也可以用一个静态的包含字典的列表，就像之前定义状态那样，
        # 然后使用关键字参数 transitions= argument 把列表传给 Machine 的初始化器。
        ...
        # 超级英雄总是时刻准备着，但他们并不总是穿好了自己的超级服装。
        self.machine.add_transition('distress_call', '*', 'saving the world',
                         before='change_into_super_secret_costume')

        # 当工作结束，超级英雄全身都汗湿了。
        # 但在他们做其他事情之前，要把最后的冒险记录下来。这是法律部门的要求。
...
现在你已经制作了一个 `NarcolepticSuperhero`（嗜睡症英雄）状态机。让我们带他出来溜两圈。
...
我们虽然不能读到蝙蝠侠的想法，但我们可以将`NarcolepticSuperhero`的状态可视化。

![batman diagram](https://user-images.githubusercontent.com/205986/104932302-c2f24580-59a7-11eb-8963-5dce738b9305.png)

具体方法可以查看[Diagrams](#diagrams)扩展。

## 不那么快速的上手

### 基础初始化

想要一个状态机跑起来是十分简单的。假设我们有一个`lump`物体 ( `Matter`类的一个实例)，你想要管理他的状态。
...
你可以用这种方式将一个状态机(_最基础版本_)绑定到`lump`上:
...
"最基础版本"是指这个状态机仅处于一个可使用状态，它没有任何功能。它以`'solid'`状态开始，但永远不会进入下一个状态，因为还没有定义任何过渡。
让我们再试一次！
...
# 然后给状态添加一些过渡。为了减轻我们的压力，一些反向的过渡就先省略。（比如freezing、condensation等）
...
注意这下`Matter`实例下的全新方法(`evaporate()`, `ionize()`, etc.)。每一个方法都能触发对应的过渡。你不需要在任何地方定义这些方法；这些过渡的名字在绑定到模型时传递到了`Machine`初始化器里 （在这个例子里，就是 `lump`实例）.
更准确的说，你的模型**不应该**包含相同名字的过渡。 `transitions`只会在名字不被使用的情况下才会将这些方便的方法加到模型里面。
如果你想要定义这些行为，请移步[FAQ](examples/Frequently%20asked%20questions.ipynb).
更进一步，一个叫`trigger`的方法会添加到你的模型里面（如果模型原来没有这个方法的话）。
当你需要动态触发的时候，这些方法能让你用名字去触发过渡。

### <a name="states"></a>状态

一个好状态机的灵魂就是一系列的状态（不好的状态机的灵魂也是）。我们通过将一个字符串的列表传入`Machine`的初始化方法来定义状态。但是在内部，状态其实是一个`State`元素。

你可以用许多方法去初始化或更改一个状态。比如：

- 将一个字符串传入`Machine`初始化阶段来进行命名，或者
- 直接初始化每一个`State`物体，或者
- 传入一个带有初始化参数的字典
