...
def dummy_ai_play(dummy: Player, discard_pile, mac_player):
    ...
    while dummy.data.stage_state == PlayerStateEnum.play:
        ...
        # 使用攻击牌
        for card in dummy.data.hand_sequence:
            if card.type == CardTypeEnum.physical_attack:
                dealed_damage = dummy.card_action.use_card(
                    card, mac_player, discard_pile
                )
                print(f"dummy使用了{card},造成了{dealed_damage}点伤害")
                break
            elif card.type == CardTypeEnum.magic_attack:
                dealed_damage = dummy.card_action.use_card(
                    card, mac_player, discard_pile
                )
                print(f"dummy使用了{card},造成了{dealed_damage}点伤害")
                break
            elif card.type == CardTypeEnum.mental_attack:
                dealed_damage = dummy.card_action.use_card(
                    card, mac_player, discard_pile
                )
                print(f"dummy使用了{card},造成了{dealed_damage}点伤害")
    ...
    while len(dummy.data.hand_sequence) > dummy.data.max_hand_sequence_num:
        for card in dummy.data.hand_sequence:
            if card.type == CardTypeEnum.armor:
                ...
                break
            elif card.type == CardTypeEnum.weapon:
                dummy.card_action.discard_card(card, discard_pile)
                break
        for card in dummy.data.hand_sequence:
            if card.type == CardTypeEnum.physical_attack:
                ...
                break
        for card in dummy.data.hand_sequence:
            if card.type == CardTypeEnum.healing:
                dummy.card_action.discard_card(card, discard_pile)
                break
